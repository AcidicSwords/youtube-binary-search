...
assert.equal(byId.get("full-video-range").disabled, true,
  "Focus also removes the Full Video boundary replacement route.");
// Event guards and rendered disabled state share the same ownership law. Even a
// synthetic caller that bypasses disabled UI cannot replace Focus-owned bounds.
byId.get("full-video-range").disabled = false;
