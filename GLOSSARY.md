...
- **Ghost Traversal** — the auxiliary operator that reads historical Traversal Trace backward and available Traversal Prospects forward while the semantic world stands still during the scan.
- **Ghost Candidate** — the provisional Address shown by a Ghost scan while accepted Current remains fixed at the Anchor.
- **Ghost scan** — transient wheel movement through one frozen Trace or Prospect source. It may reverse within that source and is never itself written as a path. Optional automatic Context recognizes each Candidate without appending Traversal Trace evidence.
- **Ghost Return** — the one historical occurrence appended on release: a jump from the live Anchor to the Address re-entered, linked to both. Backward follows its live predecessor; forward may resume the historical successors of the occurrence re-entered.
- **Ripple** — bare-Timeline `Shift+click` observation of a non-Current Address through the shared Context transport. It leaves Current, semantic history, and Traversal Trace unchanged.
...
- **Traversal Prospect** — a transient source-scoped future Address available to forward Ghost. Completed Ripple publishes distinct Start and End prospects; successful ordinary Go consumes exactly the selected identity.
- **Ripple Start Prospect / Ripple End Prospect** — the resolved start/end boundaries of a completed Ripple Context Window. Start is appended before End, so newest-first reading offers End first.
