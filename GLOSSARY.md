...
- **Traversal Trace** — the order in which the reader actually encountered source Addresses, as distinct from source time, Timeline Space and semantic history. It is append-only for ordinary reader movement and source-scoped; Ghost moves a cursor through one frozen view without manufacturing a second action type.
- **Ghost Traversal** — the auxiliary operator that moves backward and forward through one frozen Traversal Trace stream while the semantic world stands still during the scan. Ripple futures extend its forward side.
- **Ghost Candidate** — the provisional Address shown by a Ghost scan while accepted Current remains fixed at the Anchor.
- **Ghost scan** — transient wheel movement through one frozen stream. It may reverse fluidly across historical and future positions and is never itself written as a path. Optional automatic Context recognizes each Candidate without appending Traversal Trace evidence.
- **Ripple** — bare-Timeline `Shift+click` observation of a non-Current Address through the shared Context transport. It leaves Current, semantic history, Traversal Trace, and Timeline presentation unchanged.
