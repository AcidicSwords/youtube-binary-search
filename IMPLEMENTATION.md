...
Guide schema v9 names the drawn Group on the Guide as `visibleGroupId`. At most one Group is drawn, and `null` means none is: hiding the only Group is how you look at the map undeformed and unmarked, which was unreachable while the law demanded a drawn Group at all times. An explicit `null` survives the round trip because it is a choice; any other id that fails to resolve is damage and falls back to a real Group rather than blanking the map. a Group itself persists only its label and its `active` flag. v8 wrote a `visible` boolean on every Group, which let a save record two visible Groups or none, so reading one is where that ambiguity is resolved: the first Group the save marked visible becomes the named Group, falling back to `Map`. A v9 Guide naming a Group that no longer exists resolves the same way. Activity, labels, memberships, Weights, Pin Addresses and every retained identity cross the migration unchanged, and re-reading a v9 Guide changes nothing.
